...
def print_product_list(data: dict, page_no: int, page_size: int, status_filter: str = ""):
    ...
    API 响应的 data 节点中商品列表字段名为 ``detailList``（部分旧版本为
    ``list``），且接口不返回 ``total`` 总数字段。此处对两种字段名做兼容，
    并在缺少 total 时以本页条数兜底，避免列表被误判为空。
    """
    data_block = data.get("data", {}) or {}
    items = data_block.get("detailList") or data_block.get("list") or []
    total = data_block.get("total")
    total_from_api = total is not None
    if not total_from_api:
        total = len(items)
    total_pages = (total + page_size - 1) // page_size if (page_size and total) else 0
    ...
    if total_from_api:
        ...
        print(f"TikTok采集箱 - 待发布商品  |  第 {page_no} 页  |  本页 {total} 个（接口未返回总数）  |  状态: {status_label}")
    print(f"{'='*100}")
